...
def build_timeline(subj, inter_scores, ictal_scores):
    """
    Rebuilds an exact chronological representation of the EEG recordings.
    Excludes post-ictal buffer zones entirely, ensuring perfect index matching with
    inter_scores and preventing the introduction of artificial flatlines.
    """
    ...
    for edf in edfs:
        ...
        for w in range(n_win):
            ...
            if lbl == 1:
                # Ictal window: append from precomputed ictal scores
                if ictal_ptr < len(ictal_scores):
                    scores_out.append(float(ictal_scores[ictal_ptr]))
                    ictal_ptr += 1
                else:
                    scores_out.append(0.0)  # Safe fallback if bounds are exceeded
                is_ictal_out.append(True)

            elif not bfr:
                # Active interictal window: append from preprocessed interictal scores
    ...
    # Append any unconsumed preprocessed interictal scores at the end of the timeline 
    # to safeguard against slight annotation duration roundoffs
    if inter_ptr < len(inter_scores):
        diff = len(inter_scores) - inter_ptr
        print(f"    [Alignment Check] Appending {diff} unconsumed interictal windows to the end of the timeline.")
    ...
    # Extract consecutive ictal windows as structured clinical seizure events
    ...
    for i, ic in enumerate(is_ictal):
        if ic and not in_s:
            ...
            sz_ranges.append((ss_idx, i))
            in_s = False
    if in_s:
        sz_ranges.append((ss_idx, len(is_ictal)))

    n_inter_h = total_inter_s / 3600.0
    print(f"  [{subj}] Exact Timeline Aligned: {len(scores_out)} windows "
...
# Smooth the timeline with a 1-minute rolling window before running PELT
timeline_smoothed = pd.Series(timeline).rolling(window=15, min_periods=1, center=True).mean().values
algo = rpt.Pelt(model="l2", min_size=3, jump=5).fit(timeline_smoothed.reshape(-1, 1))

# ── PELT Optimizer ────────────────────────────────────────────────────────────
def run_pelt_all(signal, pen_multipliers):
    """
    Executes the dynamic programming fit step once on the 1D signal.
    Applies model="l2" (mean-shift tracking) with locked jump=5 (20s grid steps).
    Evaluates different penalty multipliers extremely fast using cached cost matrices.
    """
    n = len(signal)
    if n < 10:
        return {pm: ([], 0.0) for pm in pen_multipliers}
    
    # Estimate the robust baseline variance on the 80% lowest anomaly scores
...
def evaluate_cpd(cps, sz_ranges, n_inter_h):
    """
    Computes clinical performance metrics including True Positives, False Positives,
    False Change Points per Hour (FCP/h), and Mean Latency.
    """
    tol = TOLERANCE_S // WIN_SEC    # 7 windows (28 seconds)
    gap = MERGE_GAP_S  // WIN_SEC   # 8 windows (32 seconds)

    if not cps:
        return 0, len(sz_ranges), 0.0, float('nan')

    # Merge proximate predicted change points into single events
    merged = [cps[0]]
    for c in cps[1:]:
        if c - merged[-1] <= gap:
            merged[-1] = c
...
def main():
    ...
    model.eval()
    print(f"Joint GAE Model Loaded. Max bias = {model.encoder.conv1.bias.abs().max().item():.4f}\n")
    ...
    for subj in TEST_SUBJS:
        ...
        if ens_i_path.exists() and ens_c_path.exists():
            ...
            print(f"  Loaded cached scores: {len(ens_inter)} interictal, {len(ens_ictal)} ictal")
        else:
            print("  Precomputed cache not found. Running inference...")
            ...
            # Robust Median Absolute Deviation Normalization
            ...
            # Combine structural GAE, temporal LSTM, and spectral AEC
            ...
            print(f"  Inference completed. Saved ensemble scores.")

        # Compute unsupervised AUROC (ranking verification step)
    ...
    df.to_csv(RESULTS_DIR / "cpd_results_v6.csv", index=False)

    print("\n" + "=" * 55)
    print("MACRO SUMMARY PERFORMANCE:")
    ...
    print(f"\nDone! Perfect alignment results saved to: {RESULTS_DIR}/cpd_results_v6.csv")
