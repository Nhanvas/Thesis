"""
CPD Pipeline v7 — Temporal Smoothing Implementation
Author: Stanford/MIT Research Team (GAE & CPD for Epilepsy Research)

Changes implemented:
1. Low-Pass Temporal Smoothing: Applies a 1-minute centered moving average 
   (window=15) to filter out high-frequency transient spikes.
2. Fast PELT L2 Sweep: Locks jump=5 (20-second resolution) over the smoothed 1D signal.
3. Aligned Timeline: Preserves strict chronological interictal-ictal transitions.
"""
...
def main():
    ...
    for subj in TEST_SUBJS:
        print(f"\n{'='*70}\nProcessing Subject: [{subj}] (Smoothing)")
        ...
        if ens_i_path.exists() and ens_c_path.exists():
            ...
            print(f"  Loaded cached scores: {len(ens_inter)} inter, {len(ens_ictal)} ictal")
        else:
        ...
        # Ex 1: Apply temporal smoothing (moving average window=15, centered)
        timeline_smoothed = pd.Series(timeline).rolling(window=15, min_periods=1, center=True).mean().values

        # ── PELT Sweep on Smoothed Signal ─────────────────────────────────────
        print(f"  {'pen':>5} {'beta':>8} {'nCP':>5} {'TP':>3} {'FN':>3}"
              f" {'det%':>6} {'FCP/h':>7} {'lat_s':>7}")
        print(f"  " + "-" * 52)

        pelt_results = run_pelt_all(timeline_smoothed, pen_multipliers)
        for pm in pen_multipliers:
    ...
    df.to_csv(RESULTS_DIR / "cpd_results_v7_smoothing.csv", index=False)

    print("\n" + "=" * 55)
    print("MACRO SUMMARY PERFORMANCE (Smoothing):")
    ...
    print(f"\nDone! Results saved to: {RESULTS_DIR}/cpd_results_v7_smoothing.csv")
