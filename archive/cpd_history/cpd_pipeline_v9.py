"""
CPD Pipeline v9 — Kernel RBF Cost Implementation
Author: Stanford/MIT Research Team (GAE & CPD for Epilepsy Research)

Changes implemented:
1. Kernel RBF Cost: Uses Gaussian RBF kernel (model="rbf") in PELT 
   to track complex non-linear distributional and variance shifts.
2. Fast PELT RBF Sweep: Locks jump=5 (20-second resolution) over the raw 1D signal.
...
def run_pelt_all(signal, pen_multipliers):
    ...
    # SỬA TẠI ĐÂY: Sử dụng RBF Kernel thay cho L2 Cost
    algo = rpt.Pelt(model="rbf", min_size=3, jump=5).fit(signal.reshape(-1, 1))
...
def main():
    ...
    for subj in TEST_SUBJS:
        print(f"\n{'='*70}\nProcessing Subject: [{subj}] (RBF Cost)")
    ...
    df.to_csv(RESULTS_DIR / "cpd_results_v9_rbf.csv", index=False)

    print("\n" + "=" * 55)
    print("MACRO SUMMARY PERFORMANCE (RBF Cost):")
    ...
    print(f"\nDone! Results saved to: {RESULTS_DIR}/cpd_results_v9_rbf.csv")
