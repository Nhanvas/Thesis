...
>
> **Status 2026-09-02:** problem / method / label schema / metrics **LOCKED**. Machinery validation
> **COMPLETE and label-free** (not provisional). Results against labels are **PROVISIONAL** — the
> labels are an AI draft, not supervisor-frozen.
>
...
- **Measured.** macro-AUROC 0.6497 [0.5663, 0.7390] on 36 focal seizures, p_perm = 0.001.
- **Blocked.** The labels are near-constant within a subject (Jaccard 0.8879), so per-seizure
  attribution cannot be separated from a subject-level channel prior. This is a label limitation.
...
### 3.3 Current label file — DRAFT, PROVISIONAL
...
unchanged, and an empty `dominant_ch` whose note reads DIFFUSE becomes `generalized`. No new labelling
was performed and nothing was reconstructed from memory or chat history.

Alignment gate: all 76 rows matched `seizure_blocks.csv` on (`edf_file`, `onset_s`).

**Composition: 36 focal / 40 generalized.** Focal seizures per subject — the AUROC-eligible set:

| subject | chb03 | chb06 | chb13 | chb14 | chb15 | chb16 | chb17 | chb18 | total |
|---|---|---|---|---|---|---|---|---|---|
| focal seizures | 6 | **0** | **0** | 3 | **20** | 1 | 3 | 3 | **36** |

Mean prevalence |S|/18 = **0.073**.

### 3.4 focal vs generalized
`generalized` when the reader recorded no localisable lead channel (DIFFUSE), else `focal`. The v2 rule
"generalized if |S| ≥ 12" was not needed: the reader pass had already made the call per seizure.
...
6. **chb15 supplies 20 of 36 focal seizures (56%).** No headline over the focal set is a balanced
   8-subject result.
7. **The labels cannot test per-seizure attribution at all** (§9.3). This is the dominant limitation.
8. The spread metric for focal/generalized is a methodological negative (§9.4).
...
### 9.3 Against real labels — PROVISIONAL

36 focal seizures, seed 42, p95 aggregation. Δ = macro-AUROC − subject-constant control (D7).

| panel | n | macro-AUROC [95% CI] | AUPRC | R@\|S\| | control | Δ | p_perm |
|---|---|---|---|---|---|---|---|
| **all focal (primary)** | 36 | **0.6497** [0.5663, 0.7390] | 0.3095 | 0.1389 | 0.7758 | −0.1261 | 0.0010 |
...
D5 note: all 36 focal seizures have ≥ 3 windows, so the secondary panel coincides with the primary.
...
**What is established:** attribution is above chance — AUROC 0.6497 with p_perm = 0.001, AUPRC 0.3095
against a prevalence of 0.073 (4.2×), stable across four GAE seeds and both aggregations.
chb14 sits **below** chance (0.2672, p = 0.94).

**What is NOT established — the decisive limitation.** The subject-constant control (0.7758) beats the
per-seizure score (0.6497), Δ = −0.126. That reads as "no per-seizure information", but the labels
cannot support that reading:

| subject | n focal | distinct label sets | mean Jaccard | union size |
|---|---|---|---|---|
| chb03 | 6 | 2 | 0.8333 | 2 |
| chb14 | 3 | 2 | 0.6667 | 2 |
| chb15 | 20 | **2** | 0.9053 | 2 |
...
**Pooled within-subject Jaccard = 0.8879.** `y` is near-constant within a subject, so averaging 19
other seizures wins on noise reduction alone, regardless of what the per-seizure score contains.
Corroborating: corr(y − ȳ, s − s̄) on chb15 = **+0.089**, with only 2 distinct label sets across 20
seizures — there is almost no per-seizure variation left to measure.

⇒ **With the current labels, per-seizure attribution and a subject-level channel prior are not
distinguishable.** State this; do not report either reading as the finding.
...
1. Labels that distinguish seizures **within** a patient — target within-subject Jaccard < 0.6. The
   current draft mostly records one anatomical focus per patient.
2. Focal labels for chb06 and chb13, or explicit confirmation that all their seizures are generalized.
   Both currently contribute **0** focal seizures, so neither enters the headline.
3. Acknowledgement that chb15 supplies 20 of 36 focal seizures, so any focal headline is one-subject
   dominated.
...
- **D2 — Label source.** Results scored against the AI draft carry a PROVISIONAL banner. Official
  numbers require the supervisor's freeze. Draft labels must exist on disk with provenance; they are
  never reconstructed from memory or transcripts.
