...
**Update (2026-08) — gates PASSED, PREREG 03 approved.** Gate R-GAE **PASS**: retrained per-subject recon AUROC (mean of 5 seeds) within 0.006 of locked on every subject; macro **0.669** (band [0.64,0.70]; locked 0.671 within mean±2SD) → GAE-joint **seed-42 adopted**. Multi-seed: 3 GAE seeds {42,1,2} completed before the 200-epoch run timed out (PREREG-01 minimum; SD ≈0.0015 confirms stability; 200 epochs converge by ~epoch 40, future runs can use ~60). LSTM: 5 seeds {1,2,3,4,42} trained; Gate R-LSTM **PASS** — L1 VAL mean 0.6612 (min 0.6332), L2 recon+gamma 0.8652 → +temporal 0.8714 (Δ +0.006, net-positive but weak), L3 SD 0.0187, L4 ok; **seed-42 canonical though the weakest LSTM seed (rank 5/5) — kept by pre-registration, not cherry-picked**. PREREG 03 approved: weights re-derived on non-test (VAL grid, tie-break toward equal = repair of #19); operating-point **option A** (pre-registered rule on the full test frontier). **Remaining: `derive_weights.py` → `final_eval.py` (5-seed mean±SD) → re-lock (ROR §16).** Expect the derived temporal weight to be low; report as-is.

# PLAN_AND_STATUS — APPEND (close PHA 1 + PHA 1.5)

## Decision #25 — CLOSED (rebuild campaign complete)
Full rebuild executed and re-locked. GAE-joint reconstructed + retrained (Gate R-GAE PASS, macro recon
AUROC 0.669, 5 seeds bit-stable on VAL). LSTM temporal redesigned from principles (Gate R-LSTM PASS).
Ensemble weights re-derived on NON-TEST (PREREG 03) → **equal weights (1/3,1/3,1/3)** adopted (VAL
argmax (0.10,0.35,0.55) not adopted: flat surface, 24 ties within 0.005 → anti-overfit tie-break to
equal; TRAIN-consistent). One-pass test eval done for 5 seeds + deep seed-ensemble (tag 99). New
headline in RESULTS_OF_RECORD §16. §1–§15 marked SUPERSEDED (historical, not cited).

## Decision #26 — ADOPTED (PREREG_04: per-subject false-alarm-budget operating point)
PHA 1.5 step (a). Per-subject operating point calibrated to a pre-specified interictal FP/day budget
(balanced 40, high-sens 75), SzCORE-scored, label-free (no seizure labels in selection; tie-break
forbidden from using sensitivity). VAL guardrail A1–A3 **PASS** → applied once to test. Gain: balanced
+0.013, high-sens **+0.066** (0.711→0.776). Model remains patient-independent (per-patient false-alarm
calibration on baseline ≠ patient-specific training). BOTH shared and calibrated numbers reported
(§16.2). Adopted as the reported clinical operating result alongside the strict held-out shared result.

## Decision #27 — DEFERRED (SSL / self-supervised pretraining → Future Work)
PHA 1.5 step (b). Diagnosed as the correct lever for the SIGNAL-limited residual (chb06 window AUROC
0.442, chb14 0.545; the learned GAE/LSTM branches (~0.65) underperform hand-crafted gamma (0.755) →
self-supervised pretraining should lift them, cf. Tang et al. DCRNN+SSL → 0.875). NOT run in the thesis
due to deadline (IELTS 9 Oct, submit 15 Oct); ~1 week runway is insufficient for a 1.5–2 week uncertain
R&D. Recorded as concrete, evidence-backed Future Work (journal version). PREREG_SSL to be written then.

## STATUS after this update
- **PHA 1 (rebuild + re-lock): DONE.**
- **PHA 1.5 (a operating-point calibration): DONE.** (b SSL: deferred, Decision #27.)
- **Open / next (separate chats):**
  - **Channel attribution (§7) redesign** — NEXT CHAT. Plan agreed at design level: run on the
    per-node recon error of the ADOPTED rebuilt GAE (seed-42), pre-register 2 tests before running:
    (1) inter-seizure consistency allowing MULTIPLE clusters (Spearman rank-corr between seizures +
    permutation p; the multi-cluster form addresses the supervisor's multi-focus point), (2) spatial
    concentration via **Moran's I** on 10-20 electrode coordinates. Real method names + p-values; report
    as-is; interpretability only, never SOZ. See MASTER_HANDOFF_v2 §"Attribution".
  - **PHA 2** (LOSO; external validation on Siena) — future chat.
  - **PHA 3** (running-time, epilepsy-vs-seizure scoping, metadata stratification) — future chat.
  - **PHA 4** (writing Methods-first + minimal Phase-C demo).