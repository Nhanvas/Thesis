...
**Last updated:** 2026-09-02. **Replaces:** `MASTER_HANDOFF_v2.md`, `PLAN_AND_STATUS.md`,
...
**Done, and provisional (label-scored):**
- macro-AUROC **0.6497 [0.5663, 0.7390]**, macro-AUPRC 0.3095 (4.2× the 0.073 prevalence),
  p_perm = 0.001, over the 36 focal seizures. Stable across seeds and aggregations.

**Two honest negatives that go in the report:**
1. **The D7 control is uninformative** — within-subject label Jaccard is **0.8879**, so `y` is
   near-constant per subject and the subject-constant control wins by noise-averaging alone. With
   these labels, per-seizure attribution and a subject-level channel prior **cannot be distinguished**.
...
2. **The spread metric does not work** — normalised entropy is U-shaped in |S| (synthetic) and points
   the wrong way on real labels (focal 0.9693 > generalized 0.9594, p = 0.984). Reported as a
   methodological negative.

**Blocked on the supervisor** (three specific, evidence-backed requests in `ATTRIBUTION_SPEC.md` §9.5):
labels that vary *within* a patient (target Jaccard < 0.6); focal labels for chb06/chb13 or confirmation
that all their seizures are generalized; acknowledgement that chb15 supplies 20 of 36 focal seizures.
...
2. **Web demo (SzScan)** — `WEB_DEMO_SPEC_v4.md` wins on any conflict; `WEB_DEMO_DESIGN_SYSTEM.md` for
