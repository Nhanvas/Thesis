...
## Update (2026-08). Gate R-GAE PASS (macro 0.669, faithful) and Gate R-LSTM PASS recorded; seed-42 canonical adopted for both branches. Ensemble weights for the rebuilt pipeline are derived on non-test (PREREG 03, operating-point option A).

"Production model = deep seed-ensemble (tag 99); weights equal (Decision #26); score_ens bit-identical; PREREG_04 adopted (§16)."