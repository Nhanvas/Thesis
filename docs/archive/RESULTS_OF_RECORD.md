...
> **2026-08-14 — BASELINE-OF-RECORD is now §0 (rebuilt, reproducible pipeline). Everything in §1–§16 is PRE-REBUILD / interim history and must NOT be reported. Full method + rationale: `docs/REBUILD_BASELINE_LOCK.md`.**
...
## 0. BASELINE-OF-RECORD (LOCKED 2026-08-14) — supersedes §1, §13, §16

**This is the authoritative detection result.** Full method + rationale: `docs/REBUILD_BASELINE_LOCK.md`.
Everything below (§1–§16) is PRE-REBUILD / interim history: those numbers used an ensemble weight and
operating points **selected on the 8 test subjects** (§13.1 / §13.3) and temporal-branch components
that are **no longer reproducible** (original LSTM training code lost; a train/eval normalization bug
was later found). They are retained for provenance only. **When any §1–§16 number disagrees with §0,
§0 wins, and §0's numbers are the ones to report.**

Rebuilt pipeline (reproducible from a clean clone): equal-weight ensemble (1/3, 1/3, 1/3), canonical
seed 42 (deep 5-seed mean dropped — it dilutes anomaly peaks), label-free per-subject FP-budget
operating point (PREREG_04). SzCORE / `timescoring`, patient-independent, 8 test subjects, 76 seizures,
278.2 interictal hours.

| Operating point | Sensitivity (95% CI) | FP/day (95% CI) | TP/FN/FP |
|---|---|---|---|
| **Balanced — shared** (mag70/pen0.5) | **0.632** [0.519, 0.731] | 38.6 [35.1, 42.3] | 48/28/447 |
| Balanced — calibrated (per-subject) | 0.605 [0.493, 0.708] | 39.0 [35.5, 42.8] | 46/30/452 |
| **High-sensitivity — shared** (mag55/pen0.3) | **0.776** [0.671, 0.855] | 72.7 [67.9, 77.8] | 59/17/843 |
| High-sensitivity — calibrated (per-subject) | 0.776 [0.671, 0.855] | 72.7 [67.9, 77.8] | 59/17/843 |

- **REPORT: balanced 0.632 @ 38.6 FP/day; high-sensitivity 0.776 @ 72.7 FP/day** (shared = held-out;
  calibrated reported alongside per PREREG_04 — no gain here, so shared is the operating baseline).
- **Window tier:** macro AUROC **0.775**. **Seed stability:** 5-seed VAL standalone AUROC mean 0.648,
  SD 0.011 (Gate R-LSTM PASS).
- Latency ≈ 0 to +7 s (mildly post-onset). Do NOT claim "pre-ictal."
- **Source:** `results/retrain_v3p1/final_eval_seed42.csv`, `results/retrain_v3p1/fp_budget_locked.csv`,
  `results/retrain_v3p1/fp_budget_val_verdict.json`. Trainer `src/retrain/train_lstm_temporal_v3.py`
  (v3.1); checkpoints `data/models_retrain/`.
- **A0 (confirmatory, not adopted):** the old weight (0.40/0.35/0.25) on the *rebuilt* components →
  only ~0.63–0.645 balanced (window macro 0.767), NOT 0.750 (`results/retrain_v3p1/dec19/`). The old
  headline is not recoverable by weighting — it depended on lost components + test-selection.
- **Context:** window AUROC 0.775 exceeds the cited unsupervised CHB-MIT baseline (Yildiz 2022, 0.68).
  Supervised / TUH numbers are not comparable.

---

## 1. EVENT-LEVEL (HISTORICAL — pre-rebuild, weight/OP test-selected; SUPERSEDED by §0)
...
---
## 13. FULL DERIVATION — Decisions #19–#20 (HISTORICAL; weight + operating point were TEST-SELECTED — superseded by §0)
...
## §15b — PENDING RE-LOCK (Decision #25, 2026-08)

The locked headline (§1–§15: balanced 0.750 / FP-day 39.77; high-sens 0.829 / 71.25; window macro AUROC 0.791) remains valid and bit-exact reproducible from the frozen components and is retained as the "frozen-component" result. It is being superseded by an end-to-end reproducible rebuild (reconstructed GAE-joint + redesigned temporal branch, Decision #25), because the original temporal branch is provenance-unrecoverable. When the rebuild completes, its numbers become the headline (new §16) and the frozen-component results move to the historical/superseded record (archived, not deleted).

Robustness note (verified 2026-08): neutralising the temporal 15-window warm-up (replacing it with the per-subject interictal median) leaves the balanced point unchanged — ΔTP=0, ΔFP=0 (57/461, 0.750/39.77), per-subject identical. The warm-up affects only the window-tier temporal AUROC, never the event headline. chb17 has a separate interictal saturation artifact (lag-1 autocorr 0.998) — a data-quality issue, disclosed in Limitations.

**Gate verdicts (2026-08).** Gate R-GAE **PASS** (macro recon AUROC 0.669; every subject within 0.006 of locked; 0.671 within mean±2SD) — GAE-joint seed-42 adopted. Gate R-LSTM **PASS** (L1 VAL 0.661, L2 Δ +0.006, L3 SD 0.019, L4 ok) — temporal branch accepted. New headline pending PREREG 03 final eval → §16.

# RESULTS_OF_RECORD — §16 (INTERIM rebuild round-1; HISTORICAL — superseded by §0 / docs/REBUILD_BASELINE_LOCK.md)

**This section supersedes §1–§15 as the reported headline.** §1–§15 (the old frozen-component
pipeline, headline 0.750 / 0.829) are RETAINED as historical record but are **NO LONGER CITED** — the
temporal (LSTM) branch of that pipeline was provenance-unrecoverable, so those numbers are not
end-to-end reproducible. The numbers below ARE reproducible from committed components. Mark §1–§15 with
"SUPERSEDED — historical only, do not cite" at their headers.

Scorer for every event number below: **SzCORE any-overlap event protocol via `timescoring`.**

---

## 16.1 System of record (rebuilt pipeline)

Pipeline (unchanged flow, see `docs/Proposed_solution_updated_v5.md` / pipeline figure):
per-window connectivity graphs (broadband wPLI+AEC edges, 5-band Welch-PSD node features, top-k 20%) →
3 anomaly branches [Joint GAE reconstruction · LSTM temporal prediction-error · gamma-AEC] → per-signal
robust-z → **equal-weighted** ensemble → PELT change-point detection (`cpd_pipeline_v14`) → SzCORE.

- **Ensemble weights = (recon, temporal, gamma) = (1/3, 1/3, 1/3)** (Decision #26).
  Derived on NON-TEST (PREREG 03): the VAL argmax was (0.10, 0.35, 0.55) but the VAL macro-AUROC
  surface was flat (24 weight triples within 0.005 of the best); equal weights were chosen as the
  anti-overfit, TRAIN-consistent tie-break (repairs Decision #19's fragile margin). Single source:
  `src/ensemble_recipe.ENS_WEIGHTS`.
- **Production model = deep seed-ensemble**: the per-window ensemble score is the MEAN over 5 training
  seeds {42,1,2,3,4} (variance-reduction; GAE + gamma are deterministic, so the LSTM branch was the
  sole source of the single-seed variance). Seed-tag 99 in the artifacts.

## 16.2 Headline numbers — REPORT BOTH selection modes (PREREG_04)

Deep seed-ensemble, 8 test subjects (chb03,06,13,14,15,16,17,18), 76 seizures, 278.2 interictal hours.

**(A) SHARED operating point — strict held-out, no per-subject calibration** (option-A rule, PREREG 03):

| operating point | mag/pen | sensitivity | FP/day | TP/FN/FP |
|---|---|---|---|---|
| Balanced | mag40/pen2 | **0.618** [0.506, 0.719] | 41.58 [37.95, 45.46] | 47/29/482 |
| High-sensitivity | mag60/pen0.3 | **0.711** [0.600, 0.800] | 70.13 [65.39, 75.12] | 54/22/813 |

**(B) CALIBRATED operating point — per-subject false-alarm-budget** (PREREG_04, VAL gate A1–A3 PASS;
label-free: threshold set to a target interictal FP/day per subject, no seizure labels; model still
patient-independent — this is per-patient false-alarm calibration on baseline data, NOT patient-specific
training):

| operating point | budget | sensitivity | FP/day | TP/FN/FP |
|---|---|---|---|---|
| Balanced | 40 FP/day | **0.632** [0.519, 0.731] | 40.28 [36.71, 44.11] | 48/28/467 |
| High-sensitivity | 75 FP/day | **0.776** [0.671, 0.855] | 76.08 [71.14, 81.28] | 59/17/882 |

Calibration gain vs shared: balanced **+0.013** (near-ceiling at 40 FP/day), high-sens **+0.066**
(meaningful; the operating-point loss is recoverable at the higher budget). Reported honestly; the
oracle upper bound (each subject at its own-best point, using labels — NOT attainable) was 0.789.

## 16.3 Single-seed variance (transparency — the honest instability)

Fixed at the canonical-seed balanced/high-sens (mag,pen), across 5 seeds:

| operating point | 5-seed mean ± SD | canonical seed-42 |
|---|---|---|
| Balanced (shared) | 0.513 ± 0.178 | 0.658 |
| High-sensitivity (shared) | 0.637 ± 0.153 | 0.750 |

The SD is large and driven by two weaker LSTM seeds (seeds 2,3). The deep ensemble (§16.1) is the
variance-reduced response; the per-seed spread is reported as a limitation, not hidden.

## 16.4 Window tier (SzCORE-independent, rank-based, robust)

- Deep-ensemble **macro AUROC = 0.746**. Per-subject: chb03 0.912 · chb06 0.442 · chb13 0.811 ·
  chb14 0.545 · chb15 0.830 · chb16 0.827 · chb17 0.706 · chb18 0.892.
- 5-seed macro AUROC = **0.739 ± 0.037** (stable).

## 16.5 Diagnosis of the window→event gap (why the two-tier picture)

The `window_event_gap` diagnostic (deep ensemble) showed the shared operating point — not signal
quality — dominates the loss: pooled sens 0.618 at the shared point vs 0.789 if each subject used its
own-best point (oracle). chb06 (window AUROC 0.442, inverted connectivity) and chb14 (0.545) are
genuinely **signal-limited** (only a better signal, e.g. self-supervised pretraining, would help —
noted as Future Work). chb13/16/18 are **operating-point-limited** and partly recovered by PREREG_04.

## 16.6 Branch complementarity (why 3 branches — from the old verification, diagnostic only)

Per-subject standalone AUROC (old-component `auroc_verification.csv`, structurally indicative): recon
0.671 · temporal 0.647 · gamma 0.755 · ensemble macro 0.796. No single branch dominates: gamma best for
chb03/06/16/17, recon for chb13/18, **temporal is the ONLY strong branch for chb14 (0.823 vs gamma
0.401)**. Dropping any branch loses a subject; the ensemble beats every branch. (This justifies keeping
all three; it is a diagnostic argument, not a locked rebuilt number.)

## 16.7 Positioning vs literature (see the research report artifact)

- Window AUROC 0.746 (unsupervised, patient-independent) **exceeds** the closest unsupervised deep
  baseline, the Yıldız/Duncan convolutional VAE (≈**0.61** CHB-MIT window AUC as re-run by Yıldız
  Potter et al. 2022). NOTE: correct the governance baseline from "0.68" to **0.61** (0.83 was
  intracranial). Do not cite 0.68.
- Event sensitivity 0.63–0.78 at 40–76 FP/day, patient-independent, is far above the 2025 SzCORE
  Challenge winner (F1 0.32, sens 0.37 @ 1.34 FP/day) — a fair external reality check that
  patient-independent event detection is hard.
- Do NOT compare against patient-specific supervised methods (0.95+ sens) — different, easier problem.

## 16.8 Provenance / reproduction

Weights: `src/ensemble_recipe.ENS_WEIGHTS = (1/3,1/3,1/3)`. Components rebuilt from `src/retrain/`
(gae_joint, lstm_temporal, gamma) via `build_ens.py` → `build_seed_ensemble.py` (deep ensemble) →
`score_ens.py` (CPD+SzCORE, verified bit-identical to `szcore_eval.evaluate_subject`, see `--verify`) →
`aggregate_final.py` / `fp_budget_operating_point.py`. All result CSVs in `results/retrain/`.