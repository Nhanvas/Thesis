"""
CPD Pipeline v10 — Multivariate Change Point Detection
Author: Stanford/MIT Research Team (GAE & CPD for Epilepsy Research)

Changes implemented:
1. True Late Fusion: Feeds raw 3D [GAE Reconstruction, Temporal LSTM, Gamma AEC] 
   normalized vectors directly to PELT instead of early 1D weighted average.
2. Multidimensional PELT: Evaluates structural distance in a 3D Euclidean space, 
   successfully tracking both positive and negative mean shifts simultaneously.
3. Aligned Multivariate Timeline: Preserves strict chronological alignment for 3D tensors.
4. Independent 3D Cache: Saves multi-feature score arrays to prevent model re-inference.
"""
...
WIN_SEC     = 4  # 4-second windows
FS          = 256
BUFFER_H    = 4  # 4-hour exclusion buffer
ADJS_SUFFIX = "_topk20"
...
# ── Multivariate Timeline Reconstruction ──────────────────────────────────────
def build_timeline_multivariate(subj, recon_i, recon_c, temp_i, temp_c, gam_i, gam_c):
    """
    Reconstructs a 3D [Reconstruction, Temporal, Gamma] chronological timeline.
    Ensures perfect alignment without synthetic padding-leakage.
    """
    ...
    for edf in edfs:
        ...
        for w in range(n_win):
            ...
            if lbl == 1:
                # Ictal: Pull 3-dimensional features
                if ictal_ptr < len(recon_c):
                    scores_out.append([float(recon_c[ictal_ptr]), float(temp_c[ictal_ptr]), float(gam_c[ictal_ptr])])
                    ictal_ptr += 1
                else:
                    scores_out.append([0.0, 0.0, 0.0])
                is_ictal_out.append(True)

            elif not bfr:
                # Interictal: Pull 3-dimensional features
                if inter_ptr < len(recon_i):
                    scores_out.append([float(recon_i[inter_ptr]), float(temp_i[inter_ptr]), float(gam_i[inter_ptr])])
                    inter_ptr += 1
                    is_ictal_out.append(False)
                    total_inter_s += WIN_SEC

    # Alignment check fallback: append any residual 3D features
    if inter_ptr < len(recon_i):
        diff = len(recon_i) - inter_ptr
        print(f"    [Alignment Check] Appending {diff} unconsumed windows.")
        for i in range(inter_ptr, len(recon_i)):
            scores_out.append([float(recon_i[i]), float(temp_i[i]), float(gam_i[i])])
        is_ictal_out.extend([False] * diff)
        total_inter_s += diff * WIN_SEC
        inter_ptr = len(recon_i)

    scores_out = np.array(scores_out, dtype=np.float32)  # Shape: (T, 3)
    is_ictal   = np.array(is_ictal_out, dtype=bool)

    # Consecutive run extraction for clinical evaluation
    ...
    print(f"  [{subj}] Multivariate Aligned: {len(scores_out)} windows "
          f"({inter_ptr} inter, {ictal_ptr}/{len(recon_c)} ictal) "
...
def run_pelt_all(signal, pen_multipliers):
    """
    Fits PELT on the T x 3 matrix directly using model="l2" (Euclidean L2 distances).
    Calculates s2 as the average of robust 1D variances of the 3 dimensions.
    """
    n = len(signal)
    if n < 10:
        return {pm: ([], 0.0) for pm in pen_multipliers}
    
    # Calculate robust variance on the lowest 80% per dimension
    s2_list = []
    for d in range(3):
        col = signal[:, d]
        s2_list.append(np.var(col[col <= np.percentile(col, 80)]))
    s2 = np.mean(s2_list)
    if s2 < 1e-10:
        s2 = 1.0
        
    # Fit once on multivariate signal
...
def main():
    ...
    for subj in TEST_SUBJS:
        print(f"\n{'='*70}\nProcessing Subject: [{subj}] (Multivariate)")

        multi_i_path = SCORES_DIR / f"{subj}_multi_inter.npy"
        multi_c_path = SCORES_DIR / f"{subj}_multi_ictal.npy"

        if multi_i_path.exists() and multi_c_path.exists():
            multi_inter = np.load(str(multi_i_path))
            multi_ictal = np.load(str(multi_c_path))
            print(f"  Loaded cached 3D features: {multi_inter.shape} inter, {multi_ictal.shape} ictal")
        else:
            print("  Computing and normalizing 3D components...")
            ...
            # Robust MAD Normalization on Reconstruction
            ...
            # Stack into independent dimensions [Samples, 3]
            multi_inter = np.column_stack([z_i[:ni], t_i[:ni], g_i[:ni]])
            multi_ictal = np.column_stack([z_c[:nc], t_c[:nc], g_c[:nc]])
            
            np.save(str(multi_i_path), multi_inter)
            np.save(str(multi_c_path), multi_ictal)
            del z_i, z_c, t_i, t_c, g_i, g_c
            print(f"  Saved 3D features: {multi_inter.shape} inter, {multi_ictal.shape} ictal")

        # Early fusion equivalent AUROC computation for ranking verification only
        W_R, W_T, W_G = 0.35, 0.30, 0.35
        ens_inter = W_R*multi_inter[:,0] + W_T*multi_inter[:,1] + W_G*multi_inter[:,2]
        ens_ictal = W_R*multi_ictal[:,0] + W_T*multi_ictal[:,1] + W_G*multi_ictal[:,2]
        y     = np.concatenate([np.zeros(len(ens_inter)), np.ones(len(ens_ictal))])
        auroc = roc_auc_score(y, np.concatenate([ens_inter, ens_ictal]))
        print(f"  Ensemble AUROC (Reference): {auroc:.4f}")
        del ens_inter, ens_ictal

        # ── Reconstruct 3D Timeline ───────────────────────────────────────────
        timeline, is_ictal, sz_ranges, n_inter_h = build_timeline_multivariate(
            subj, multi_inter[:,0], multi_ictal[:,0],
            multi_inter[:,1], multi_ictal[:,1],
            multi_inter[:,2], multi_ictal[:,2]
        )
        del multi_inter, multi_ictal

        if len(timeline) == 0:
            print(f"  Warning: Reconstructed timeline is empty. Skipping {subj}.")
            continue

        # ── Multivariate PELT Sweep ───────────────────────────────────────────
    ...
    df.to_csv(RESULTS_DIR / "cpd_results_v10_multivariate.csv", index=False)

    print("\n" + "=" * 55)
    print("MACRO SUMMARY PERFORMANCE (Multivariate):")
    ...
    print(f"\nDone! Multivariate results saved to: {RESULTS_DIR}/cpd_results_v10_multivariate.csv")
