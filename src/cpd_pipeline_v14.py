...
# Phase-C / C1: swappable pre-CPD smoother. LOCKED DEFAULT "ma15" == the v13/v14
# 15-window centered moving average -> reproduces the §0/rlg grids BYTE-IDENTICAL.
# Alternatives target FP/precision by suppressing ISOLATED interictal spikes that a
# mean merely smears into a change point (a median removes them; the seizure burst,
# being multi-window, survives). Set this module global before scoring to switch;
# score_ens/detect_* call _smooth() with no method arg and read it at call time.
SMOOTHER = "ma15"        # one of: "ma15" (locked), "median15", "median9", "median5"

# Phase-C slope-gate (onset-rate CP filter). 0 = OFF -> byte-exact §0/rlg. When >0,
# keep only change-points whose local SIGNED rise (onset = rise; interictal FP = high-
# level plateau ~ flat) >= the percentile of background (interictal) CP rises. Mirrors
# the magnitude filter's architecture; set this module global before scoring to switch.
MIN_SLOPE_PCT = 0


# ----------------------------------------------------------------------------
def _smooth(score_timeline, method=None):
    x = pd.Series(np.asarray(score_timeline, dtype=float))
    m = method or SMOOTHER
    if m == "ma15":
        return x.rolling(window=SMOOTH_WIN, min_periods=1, center=True).mean().values
    if m.startswith("median"):
        w = int(m[6:]) if len(m) > 6 else SMOOTH_WIN
        return x.rolling(window=w, min_periods=1, center=True).median().values
    raise ValueError(f"unknown SMOOTHER {m!r} (ma15|medianN)")
...
def _cp_slope(signal, c, L):
    """SIGNED local rise at change-point c (mean of the L windows after minus before).
    Onset CPs rise (positive); high-level interictal plateaus are ~flat (near zero)."""
    a0, a1, b0, b1 = max(0, c - L), c, c, min(len(signal), c + L)
    if a1 <= a0 or b1 <= b0:
        return 0.0
    return float(signal[b0:b1].mean()) - float(signal[a0:a1].mean())


def _slope_filter(signal, cps, local_win, inter_mask, pct):
    """Keep CPs whose signed rise >= percentile(pct) of BACKGROUND (interictal) CP rises.
    Label-free (threshold from interictal CPs, or all if no mask). pct<=0 -> no-op."""
    if not (pct and pct > 0 and cps):
        return list(cps)
    sl = {c: _cp_slope(signal, c, local_win) for c in cps}
    if inter_mask is not None and np.any(inter_mask):
        bg = [v for c, v in sl.items() if inter_mask[c]]
    else:
        bg = list(sl.values())
    thr = float(np.percentile(bg, pct)) if bg else 0.0
    return [c for c in cps if sl[c] >= thr]
...
def detect_changepoints(score_timeline, pen_mult, min_mag_pct=60, local_win=15,
    ...
    # Phase-C slope-gate (default OFF via MIN_SLOPE_PCT=0 -> byte-exact)
    if MIN_SLOPE_PCT and MIN_SLOPE_PCT > 0 and cps:
        cps = _slope_filter(smoothed, cps, local_win, inter_mask, MIN_SLOPE_PCT)
    return cps, smoothed
