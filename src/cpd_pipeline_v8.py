"""
CPD Pipeline v8 — Robust L1 Cost Implementation
Author: Stanford/MIT Research Team (GAE & CPD for Epilepsy Research)

Changes implemented:
1. Robust L1 Cost: Uses absolute deviation cost (model="l1") in PELT 
   to ignore sparse, heavy-tailed impulsive outliers.
2. Fast PELT L1 Sweep: Locks jump=5 (20-second resolution) over the raw 1D signal.
...
def run_pelt_all(signal, pen_multipliers):
    ...
    # SỬA TẠI ĐÂY: Sử dụng L1 Cost thay cho L2 Cost
    algo = rpt.Pelt(model="l1", min_size=3, jump=5).fit(signal.reshape(-1, 1))
...
def main():
    ...
    for subj in TEST_SUBJS:
        print(f"\n{'='*70}\nProcessing Subject: [{subj}] (L1 Cost)")
    ...
    df.to_csv(RESULTS_DIR / "cpd_results_v8_l1.csv", index=False)

    print("\n" + "=" * 55)
    print("MACRO SUMMARY PERFORMANCE (L1 Cost):")
    ...
    print(f"\nDone! Results saved to: {RESULTS_DIR}/cpd_results_v8_l1.csv")
