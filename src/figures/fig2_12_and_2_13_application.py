"""
Fig 2.12 and Fig 2.13 - the review application, and why it recomputes.

Two figures, drawn by one script because they share one source file.

Fig 2.12  The application's components, and the three paths it must never take. The
...
Fig 2.13  The measurement that forces (a). Artifact rejection removes background
...
Every proportion in Fig 2.13 is read from results/diagnostics/timeline_composition.csv
...
DROPPED_FRAC = (1.0 - STORED / ELIGIBLE) if "dropped_fraction" not in df.columns \
    else float(row["dropped_fraction"])
...
print(f"  dropped by rejection   : {DROPPED_FRAC:.3f} of eligible positions")
...
# ---------------------------------------------------------------------------
apply_rc()
figa = plt.figure(figsize=(10.4, 4.9))
axa = figa.add_axes([0.02, 0.02, 0.96, 0.96])
axa.set_xlim(0, 100)
axa.set_ylim(0, 100)
axa.axis("off")
...
# === Fig 2.13 — why replay is impossible ===================================
figb = plt.figure(figsize=(10.4, 3.7))
axb = figb.add_axes([0.02, 0.02, 0.96, 0.96])
axb.set_xlim(-0.30, 1.06)
axb.set_ylim(-0.62, 3.05)
axb.axis("off")
...
print("\ncaption for Figure 2.12:")
print(f"  Architecture of the review application. The application recomputes "
      f"anomaly scores on the continuous recording, retaining every window. It "
      f"never reads the seizure annotations, any timeline rebuilt from them, or "
      f"the pre-split background and seizure arrays, all three of which would "
      f"make a label-free claim false. Figure 2.13 gives the measurement that "
      f"forces this design.")

print("\ncaption for Figure 2.13:")
print(f"  Why a stored score array cannot be replayed on a time axis. Artifact "
      f"rejection removes {DROPPED_FRAC * 100:.0f} % of the background windows "
      f"of {EXAMPLE} without recording their positions, so the stored array "
      f"holds {STORED:,} scores for {ELIGIBLE:,} timeline positions. A "
      f"reconstructed timeline shifts the surviving scores out of their "
      f"original positions and, once the array is exhausted at "
      f"{EXHAUST * 100:.0f} % of the recording, fills the remainder by "
      f"resampling. Across the {N_SUBJ} held-out patients "
      f"{SUB_MEAN * 100:.1f} % of each reconstructed timeline carries a "
      f"resampled score on average, ranging from {SUB_MIN * 100:.1f} % to "
      f"{SUB_MAX * 100:.1f} %. The resampling preserves the marginal "
      f"distribution of each patient's own background but not its temporal "
      f"autocorrelation. The false-alarm rate is measured on these timelines, "
      f"and the direction of the resulting bias is not established.")
