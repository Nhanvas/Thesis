...
def draw_graph(ax, A, channels, title):
    n = len(channels)
...
def main():
    ...
    print(f"[Fig 2.5] window {WINDOW_IDX} own density: fixed={this_window_fixed_density:.3f}, "
          f"topk={this_window_topk_density:.3f} (title uses the Task E mean over "
          f"{n_sampled} windows, not this window's own value)")

    channels = P.COMMON_CHANNELS
    fig, axes = plt.subplots(2, 2, figsize=(12, 11.5))

    im0 = axes[0, 0].imshow(A_fixed, vmin=0, vmax=A_fixed.max(), cmap="viridis")
    ...
    # bare data value, not a narrative title (brief §1 rule 5).
    axes[0, 0].set_title(f"density = {dens_fixed:.3f}")
    fig.colorbar(im0, ax=axes[0, 0], shrink=0.85, pad=0.02)
    draw_graph(axes[0, 1], A_fixed, channels, f"density = {dens_fixed:.3f}")

    im1 = axes[1, 0].imshow(A_topk, vmin=0, vmax=A_topk.max(), cmap="viridis")
    ...
    axes[1, 0].set_title(f"density = {dens_topk:.3f}")
    fig.colorbar(im1, ax=axes[1, 0], shrink=0.85, pad=0.02)
    draw_graph(axes[1, 1], A_topk, channels, f"density = {dens_topk:.3f}")
